...
static int 
minix_getdents(struct m_inode *inode, struct file *filp, struct dirent *dirent, int count)
{
...
static int 
minix_getdents64(struct m_inode *inode, struct file *filp, struct dirent64 *dirent, int count)
{
	unsigned int offset, i;
	char c;
	struct buffer_head *bh;
...
int 
sys_getdents(unsigned int fd, struct dirent *dirp, unsigned int count)
{
	struct file *file;
...
int 
sys_getdents64(unsigned int fd, struct dirent64 *dirp, unsigned int count)
{
	struct file *file;
	struct m_inode *inode;
	if (fd >= NR_OPEN || !(file = current->filp[fd]) || !(inode = file->f_inode))
