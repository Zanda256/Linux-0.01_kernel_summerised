/* use elf binary instead of a.out */
...
inline void 
cp_block(const void *from,void *to, int size)
{
int d0,d1,d2;
 ...
 */
static unsigned long 
copy_strings(int argc,char ** argv,unsigned long *page, unsigned long p)
{
	int len,i;
	char *tmp;
 ...
 */
struct buffer_head* 
read_file_block_ind(int dev, int ind, int block_num)
{
 ...
 */
struct buffer_head* 
read_file_block(struct m_inode* inode,int block_num)
{
...
int 
copy_section(struct m_inode* inode,Elf32_Off from, Elf32_Addr dest,Elf32_Word size)
{
...
inline int 
create_bss_section(Elf32_Addr dest,Elf32_Word size)
{
	while (size--)
 ...
 */
int 
load_elf_binary(struct m_inode *inode, struct buffer_head* bh, bin_section* bs)
{
	Elf32_Ehdr ex;
	Elf32_Shdr sect;
	long nsect;
